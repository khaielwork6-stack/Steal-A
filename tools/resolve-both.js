...
// GLUE may also be an array of arrays: one glue list per hunk, in order.
const parsed = glueJson ? JSON.parse(glueJson) : [];
const perHunk = Array.isArray(parsed[0]);
let hunk = 0;
...
while (i < lines.length) {
  if (lines[i].startsWith("<<<<<<< ")) {
    ...
    const glue = perHunk ? parsed[hunk] || [] : parsed;
    hunk++;
